export const contact = [
    {
        'Эл. почта': `mailto: bogucharskiy.s@yandex.ru`
    },
    {
        'Telegram': `https://telegram.me/Severinz`
    },
    {
        'Discord': `https://discord.com/users/214732781945618442`
    },
    {
        'GitHub': `https://github.com/Diatom/`
    }
]

export const contactIbri = [
    {
        'bogucharskiy.s@yandex.ru': `mailto: bogucharskiy.s@yandex.ru`
    },
    {
        'Telegram: @Severinz': `https://telegram.me/Severinz`
    },
    {
        '+7 (903) 186-93-32': `tel:+79031869332`
    }
]

export const bloglist = [
    {
        'name': `Общество`,
        'dataindex': 'social',
        'desc': 'Блог о социальных темах, обществе, мире и государствах',
    },
    // {
    //     'name': `Мир`,
    //     'dataindex': 'world',
    //     'desc': 'Блог о путешествиях и окружающем мире',
    // },
]

export const list = [
    {
        'id': '0',
        'dataindex': 'social/gosudarstvo-kak-brend',
        'path': './post/social/gosudarstvo-kak-brend.md',
        'h3': 'Государство как бренд',
        'src': '/images/art-images/social/gosudarstvo-kak-brend-by-pavel-neznanov.jpg',
        'alt': 'By Pavel Neznanov',
        'date': 'Дата публикации: 14 марта 2024',
        'p': 'Есть ли у личности идентичность с государством?'
    },
    {
        'id': '1',
        'dataindex': 'social/grazhdane-est-gosudarstvo',
        'path': './post/social/grazhdane-est-gosudarstvo.md',
        'h3': 'Граждане есть государство',
        'src': '/images/art-images/social/grazhdane-est-gosudarstvo-by-pavel-neznanov.jpg',
        'alt': 'By Pavel Neznanov',
        'date': 'Дата публикации: 14 марта 2024',
        'p': 'Дистанция между управленцами и рядовыми гражданами государства'
    },
    {
        'id': '2', 
        'dataindex': 'social/konkurenciya-istoriya',
        'path': './post/social/konkurenciya-istoriya.md',
        'h3': 'Конкуренция, история',
        'src': '/images/art-images/social/konkurenciya-istoriya-by-Nikita-Ermilov.jpg',
        'alt': 'By Nikita Ermilov',
        'date': 'Дата публикации: 14 марта 2024',
        'p': 'Диалог не воспринимается конечным способом решения проблем'
    },
]
// export const list = [
//     {
//         'id': '0',
//         'dataindex': 'gosudarstvo-kak-brend',
//         'path': './post/gosudarstvo-kak-brend.md',
//         'h3': 'Государство как бренд',
//         'src': './images/art-images/gosudarstvo-kak-brend-by-pavel-neznanov.jpg',
//         'alt': 'By Pavel Neznanov',
//         'date': 'Дата публикации: 14 марта 2024',
//         'p': 'Есть ли у личности идентичность с государством?'
//     },
//     {
//         'id': '1',
//         'dataindex': 'grazhdane-est-gosudarstvo',
//         'path': './post/grazhdane-est-gosudarstvo.md',
//         'h3': 'Граждане есть государство',
//         'src': './images/art-images/grazhdane-est-gosudarstvo-by-pavel-neznanov.jpg',
//         'alt': 'By Pavel Neznanov',
//         'date': 'Дата публикации: 14 марта 2024',
//         'p': 'Дистанция между управленцами и рядовыми гражданами государства'
//     },
//     {
//         'id': '2', 
//         'dataindex': 'konkurenciya-istoriya',
//         'path': './post/konkurenciya-istoriya.md',
//         'h3': 'Конкуренция, история',
//         'src': './images/art-images/konkurenciya-istoriya-by-Nikita-Ermilov.jpg',
//         'alt': 'By Nikita Ermilov',
//         'date': 'Дата публикации: 14 марта 2024',
//         'p': 'Диалог не воспринимается конечным способом решения проблем'
//     },
// ]